...
//	int number_of_split = 1;
	...
	long TARGET_BLOCK_SIZE = 1024 * 1024 * 12;
...
//	}
	...
	char buffer[TAR_BLOCK_SIZE * 2 + 1];
	...
	current = buffer;
	 {
		 ...
		 */
		int count = 0;
		while ((size = source->read((unsigned char *)current, TAR_BLOCK_SIZE)) > 0) {
			*(current + size) = '\0';
			bytes_read += size;
			total_read += size;
...
//			if ((TARGET_BLOCK_SIZE - bytes_read) <= (TAR_BLOCK_SIZE * 3)) {
			...
			while ((pos = strstr(pos, "<page>"))) {
				...
				/*
				 * anything we have before this point we dump it
				 */
				if (dump_flag && pre != NULL) {
					cout << string(pre, pos - pre);
					dump_flag = 0;
					pre = pos;
				}
...
//				pos += 7;
				...
				char *ns = between(pos, "ns");
				int ns_id = ns ? atoi(ns) : 0;
				if (ns_id != 0)
					{
					cout << string(pos, end - pos) << endl;
					}
				else {
...
//			}
			...
			int processed_bytes = pos - buffer;
			bytes_forward = 0;
			if (processed_bytes > 0 && processed_bytes < size) {
				bytes_forward = size - processed_bytes;
				memmove(buffer, pos, bytes_forward);
			}
			else
				*buffer = '\0';
			current = buffer + bytes_forward;
//			current += size;
			//cout << buffer;
		}

		if (bytes_forward > 0)
			cout << string(buffer, bytes_forward) << endl;
